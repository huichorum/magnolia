        ...
        <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
            ...
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <img src="src/banner/banner_incio1.jpg" class="d-block w-100" alt="..." width="700px"
                        height="500px">
                </div>
                <div class="carousel-item">
                    <img src="src/banner/banner_inicio2.jpg" class="d-block w-100" alt="..." width="700px"
                        height="500px">
                </div>
                <div class="carousel-item">
                    <img src="src/banner/banner_inicio3.jpg" class="d-block w-100" alt="..." width="700px"
                        height="500px">
                </div>
            </div>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    if (!isset($_SESSION['Nusuario'])) {
                        echo "<script>alert('Para ver los productos del carrito necesitas iniciar sesión')</script>";
                        echo "<script>window.location.replace('/Magnolia/registro.php');</script>";
                    } else {
                        $cuenta = 0;
                        $consulta = "SELECT IDropa,IDcategoria,Producto,Precio,Cantidad,Total,Imagen FROM carrito WHERE Nusuario='" . $_SESSION['Nusuario'] . "'";
                                ...
                                <form action="php/conexion.php" method="POST">
                                    <tr>
                                        <td><button type="button" class="btn"
                                                onclick="bajar('<?php echo $row['IDropa'] ?>')">-</button></td>
                                        <td><input type="number" readonly id="<?php echo $row['IDropa'] ?>" name="cantidad"
                                                value="<?php echo $row['Cantidad'] ?>" /></td>
                                        <td><button type="button" class='btn'
                                                onclick="subir('<?php echo $row['IDropa'] ?>')">+</button></td>
                                    </tr>
                            </table>
                            </td>
                            <?php
                            echo "<td>" . $row['Producto'] . "</td>";
                            echo "<td><img src='data:image/png;base64, " . base64_encode($row['Imagen']) . "'alt='" . $row['Producto'] . "' width='100px' height='120px'></td>";
                            echo "<td>$"; ?>
                            <input type="number" readonly id="T<?php echo $row['IDropa'] ?>" name="total"
                                value="<?php echo $row['Total'] ?>" />
                            <?php echo "</td></tr>"; ?>
                            <input type="number" hidden id="P<?php echo $row['IDropa'] ?>"
                                value="<?php echo $row['Precio'] ?>" />
                            <input hidden name="ropa" value="<?php echo $row['IDropa'] ?>" />
                            <input hidden name="producto" value="<?php echo $row['Producto'] ?>" />
                            <input hidden name="user" value="<?php echo $_SESSION['Nusuario'] ?>" />
                            <input hidden type="submit" name="update" id="B<?php echo $row['IDropa'] ?>" />
                            </form>
                            <?php
                            $cuenta += $row['Total'];
                        }
                    ...
                    <form action="pago.php" method="POST">
                        <tr>
                            ...
                            <td>$<input type="number" readonly name="total" value="<?php echo $cuenta ?>" /></td>
                        </tr>
                        <tr>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td><button class="btn btn-dark" name="pagar" type="submit">Pagar</button>
                    </form>
                    </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <form action="php/conexion.php" method="POST">
                                <button class="btn btn-light" name="borrar_carrito" type="submit">Vaciar
                                    carrito</button>
                            </form>
                        </td>
                    </tr>
